...
void Polygon::saveAsMetaPost()
{
	...
	double scaley = 1.0;
	double scale=(scalex <= scaley)?scalex:scaley;
	double offsetx=0-_xmin*scale,  offsety=0-_ymin*scale;
	...
	ftxt.open("test_in.txt", ios::out);

	Monopolys::iterator itms = _mpolys.begin();
	ftxt << _mpolys.size() << endl; 
	for(; itms != _mpolys.end(); itms ++)
	{
		Monopoly::reverse_iterator itm = itms->rbegin();
		ftxt << itms->size() << endl;
		for(; itm != itms->rend(); itm++)
		{
			PointbaseMap::iterator itp = _points.begin();
			int i = 1;
			while (i < *itm)
...
bool Polygon::ChangeToConset(rcContourSet &cset)
{
	int maxContours;
	//sum = 0;
	maxContours = _mpolys.size(); 
	...
	{		
		...
		for(; itm != itms->rend(); itm++)
		{
			...
			cont->verts[index ++] = 0;
			cont->verts[index ++] = y;
			cont->verts[index ++] = 131072;
		}
		cont->reg = cset.nconts - 1;
		cont->area = cset.nconts - 1;
	}
	return true;
}
