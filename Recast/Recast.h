...
struct rcContourSet
{
	rcContour* conts;	///< An array of the contours in the set. [Size: #nconts] 轮廓数组
	int nconts;			///< The number of contours in the set. 轮廓的数目
	float bmin[3];  	///< The minimum bounds in world space. [(x, y, z)]
	float bmax[3];		///< The maximum bounds in world space. [(x, y, z)]
	float cs;			///< The size of each cell. (On the xz-plane.) 每个块的大小
	float ch;			///< The height of each cell. (The minimum increment along the y-axis.) 块的高度
	int width;			///< The width of the set. (Along the x-axis in cell units.) 集合的宽度
	int height;			///< The height of the set. (Along the z-axis in cell units.) 集合的高度
...
//#debug 
void MyOutputContourData(fstream *f, rcContour rcCon);
#if 0
//#debug 
void MyOutputContourData(fstream f, rcContour rcCon)
{
	for(int i = 0; i < rcCon.nverts; i ++)
	{
		f << rcCon.verts[i] << endl;
	}
	f << endl;
	for(int j = 0; j < rcCon.nrverts; j ++)
	{
		f << rcCon.rverts[j] << endl;
	}
	f << endl;
}
#endif
//#debug 
void MyOutputContourSet(fstream *f, rcContourSet *rcConSet);
#if 0
void MyOutputContourSet(fstream f, rcContourSet rcConSet)
{
...
//#debug 
void MyOutputPolyMesh(fstream *f, rcPolyMesh *rcPmesh);
#if 0
void MyOutputPolyMesh(fstream *f, rcPolyMesh *rcPmesh)
{
	(*f) << "Num of Poly vertices: " <<  rcPmesh->nverts <<endl;
	for (int i = 0; i < rcPmesh->nverts; i ++)
	{
		(*f) << rcPmesh->verts[i] << " ";
		if (i % 3 == 0 && i)
		{
